...
const InvoicePDF = ({ order, user }: InvoicePDFProps) => {
    ...
    return (
        <Document>
            ...
            >
                <InvoiceHeader order={order} user={user} />
                <Text
                    style={{
...
const InvoiceHeader = ({ order, user }: { order: IOrder; user: ICustomer | null }) => {
    return (
        <View
            style={{
                flexDirection: 'row',
        ...
        >
            ...
            <View style={{ alignItems: 'center', gap: 2, maxWidth: '50%' }}>
                ...
                <Text>
                    <Text style={{ fontWeight: 'medium' }}>Khách hàng:</Text> {user?.name}
                </Text>
            </View>
        </View>
    )
